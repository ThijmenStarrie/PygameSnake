...
def random_apple():
    while True:
        ...
        for s in snake_xy:
            ...
            continue
        break
    pygame.draw.rect(DISPLAYSURF, apple_color, (xmargin + applex * square_size + apple_margin,
...
def render_snake():
    ...
    print(snake_xy[-1])
    pygame.draw.rect(DISPLAYSURF, snake_head_color, (xmargin + head_x * square_size + snake_margin,
                                                     ymargin + head_y * square_size + snake_margin,
                                                     square_size - 2 * snake_margin, square_size - 2 * snake_margin))
    if len(snake_xy) > 1:
        last_head_x, last_head_y = snake_xy[-2]
        pygame.draw.rect(DISPLAYSURF, snake_head_color, (xmargin + last_head_x * square_size + snake_margin,
                                                         ymargin + last_head_y * square_size + snake_margin,
...
update_score(score)
background_grid()
pygame.draw.rect(DISPLAYSURF, apple_color, (xmargin + apple_xy[0] * square_size + apple_margin,
                                                ymargin + apple_xy[1] * square_size + apple_margin,
                                                square_size - 2 * apple_margin, square_size - 2 * apple_margin))
